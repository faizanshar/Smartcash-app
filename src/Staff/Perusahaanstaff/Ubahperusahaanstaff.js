...
import { Text, View, Image, TextInput, TouchableOpacity, ToastAndroid, ScrollView, StatusBar } from 'react-native'
...
class Ubahperusahaanstaff extends Component {
    ...
    componentDidMount() {
      ...
      editPerusahaan = () => {
        ...
        fetch(`https://smartcash2.herokuapp.com/api/supplier/update/${this.props.route.params.item.id}`, {
          method: 'POST',
          headers: {
        ...
        return (
            <View style = {styles.container}>
                <ScrollView>
                    ...
                    <TextInput placeholder = {' perusahaan'} style = {styles.input} value = {this.state.perusahaan} onChangeText = {(input) => this.setState({perusahaan:input})}/>
                    <TextInput placeholder = {' telpon'} style = {styles.input2} value = {this.state.telpon} onChangeText = {(input) => this.setState({telpon:input})}/>
                    <TextInput placeholder = {' alamat'} style = {styles.input2} value = {this.state.alamat} onChangeText = {(input) => this.setState({alamat:input})}/>
                    <TouchableOpacity style = {styles.touchubah} onPress = {() => this.editPerusahaan()}>
